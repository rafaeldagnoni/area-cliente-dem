...
// ─── FONTES ───────────────────────────────────────────────────────────────────
const FONT_URL =
  "https://fonts.googleapis.com/css2?family=Barlow:wght@400;500;600;700;800;900&family=Barlow+Condensed:wght@400;600;700;800&family=JetBrains+Mono:wght@400;500&display=swap";
...
type DreRow = {
  key: string;
  nivel: number;
  tipo: "grupo" | "subgrupo" | "item" | "resultado" | "saldo";
  pref: string;
  sinal?: number;
  filhos?: string[];
  cor?: "blue" | "gold" | "green";
...
const DRE_ROWS: DreRow[] = [
  { key: "Receita de Vendas", nivel: 0, tipo: "grupo", pref: "(+)", sinal: 1, filhos: ["Receita Outros", "Receita Serviço", "Receita Venda/Revenda", "Loja Virtual"] },
  ...
  { key: "Deduções de Vendas", nivel: 0, tipo: "grupo", pref: "(-)", sinal: -1, filhos: ["Comissões de vendas", "Devolução ou Cancelamento NF", "Impostos (Federais, Estaduais, Municipais)"] },
  ...
  { key: "Custo dos Produtos Vendidos", nivel: 0, tipo: "grupo", pref: "(-)", sinal: -1, filhos: ["CMV/CPV - Custo Mercadoria Vendida", "Custos Variáveis de Operação", "Devolução de Matéria Prima e Insumo", "Mão de obra terceirizada"] },
  ...
  { key: "Despesas Variáveis", nivel: 0, tipo: "grupo", pref: "(-)", sinal: -1, filhos: ["Fretes e Combustíveis (venda)", "Gastos com Veículos", "Manutenção de Equipamentos", "Outros", "Taxa de Boletos | Cartão"] },
  ...
  { key: "Gastos fixos (custos fixos + despesas fixas)", nivel: 0, tipo: "grupo", pref: "(-)", sinal: -1, filhos: ["Gasto com Pessoal - Adm", "Gasto com pessoal - Prod/Oper", "Despesas Operacionais"] },
  {
    key: "Gasto com Pessoal - Adm",
    nivel: 1,
    tipo: "subgrupo",
    pref: "(-)",
    sinal: -1,
    filhos: [
      ...
      "INSS - Adm",
      "IRRF - Adm",
      "Multa Rescisória - Adm",
      "Outros Benefícios - Adm",
      "Outros Folha - Adm",
      "Pesão Alimentícia - Adm",
      "Salário (s/ encargos) - Adm",
      "Seguro de Vida - Adm",
      "Vale Refeição - Adm",
      "Vale Transporte - Adm",
    ],
  ...
  {
    key: "Gasto com pessoal - Prod/Oper",
    nivel: 1,
    tipo: "subgrupo",
    pref: "(-)",
    sinal: -1,
    filhos: [
      "13° Salário - Prod/Oper",
      "Benefícios - Prod/Oper",
      "Refeições - Colaboradores",
      "Convênio Médico - Prod/Oper",
      "Férias - Prod/Oper",
      "FGTS - Prod/Oper",
      "INSS - Prod/Oper",
      "IRRF - Prod/Oper",
      "Multa Rescisória - Prod/Oper",
      "Outros Folha Prod/Oper",
      "Salário (s/ encargos) - Prod/Oper",
    ],
  },
...

  {
    ...
    filhos: [
      ...
      "Consertos e reparos - Adm",
      "Contabilidade, Jurídico, Consultoria",
      "Estrutural (energia, água, seguro)",
  ...
  { key: "Impostos Sob Lucro", nivel: 0, tipo: "grupo", pref: "(-)", sinal: -1, filhos: ["CSLL", "IRPJ"] },
  ...
  { key: "Distribuição de Lucro", nivel: 0, tipo: "grupo", pref: "(-)", sinal: -1, filhos: ["Distribuição de lucro", "Gratificações a funcionários"] },
  ...
  { key: "Investimentos e Financiamentos", nivel: 0, tipo: "grupo", pref: "(-)", sinal: -1, filhos: ["Compra de Ativo Imobilizado", "Empréstimos e Financiamentos (Saída)"] },
...
function TabelaFinanceira({
  ...
  const toggleExpand = (key: string) => setExpandidos((prev) => ({ ...prev, [key]: !prev[key] }));

  const receitaBase = VALORES["Receita de Vendas"]?.[mesSel] || 1;

  const findParent = (rowsAll: DreRow[], childKey: string) => {
  ...
  const rowColor = (tipo: DreRow["tipo"], cor?: DreRow["cor"]) => {
  ...
  const mesesExibidos = mostrarAno
    ? MESES.map((m, i) => ({ label: m, idx: i })).filter((m) => (VALORES["Receita de Vendas"][m.idx] || 0) > 0)
      ...
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5, fontFamily: "'Barlow',sans-serif" }}>
        <thead>
            ...
            {mesesExibidos.map((m) => (
              <th
                ...
                style={{
                  ...
                  fontSize: 12,
                  letterSpacing: 1,
                  borderLeft: `1px solid ${C.gray700}`,
                }}
              >
                {m.label.toUpperCase()}
              </th>
          ...
          <tr style={{ background: C.gray900, borderBottom: `2px solid ${C.red}` }}>
            <th style={{ padding: "6px 14px", textAlign: "left", color: C.gray300, fontSize: 10, letterSpacing: 1 }} />
            {mesesExibidos.map((m) => (
              <span key={`hdr-${m.idx}`} style={{ display: "contents" }}>
                <th style={{ padding: "6px 8px", textAlign: "right", color: C.gray300, fontSize: 10, borderLeft: `1px solid ${C.gray700}` }}>
                  R$ TOTAL
                </th>
                <th style={{ padding: "6px 8px", textAlign: "right", color: C.gray300, fontSize: 10 }}>
                  %
                </th>
              </span>
            ))}
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {hasChildren && <span style={{ color: C.gray500, fontSize: 10, marginLeft: 4 }}>{isOpen ? "▲" : "▼"}</span>}
                </td>

                {mesesExibidos.map((m) => {
                  const v = VALORES[row.key]?.[m.idx] ?? 0;
                  const pct = fmtPct(v, receitaBase);
                  const isNeg = v < 0;

                  return (
                    <span key={`cells-${row.key}-${m.idx}`} style={{ display: "contents" }}>
                      <td
                        style={{
                          padding: "7px 8px",
                          textAlign: "right",
...
// ─── VIEW OVERVIEW ────────────────────────────────────────────────────────────
function OverviewView({ mesSel }: { mesSel: number }) {
  ...
  const KPI = ({ label, value, sub, trend, color }: KPIProps) => (
    <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 8, padding: "18px 20px", borderTop: `3px solid ${color || C.red}` }}>
      ...
      {trend !== undefined && (
        <div style={{ marginTop: 8, fontSize: 11, color: parseFloat(String(trend)) >= 0 ? C.green : C.red, fontWeight: 600 }}>
          {parseFloat(String(trend)) >= 0 ? "▲" : "▼"} {Math.abs(parseFloat(String(trend)))}% vs mês ant.
        </div>
      )}
    </div>
  );
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: 20 }}>
        <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 8, padding: 20 }}>
          <div style={{ fontFamily: "'Barlow Condensed',sans-serif", fontWeight: 700, fontSize: 13, letterSpacing: 1, color: C.dark, textTransform: "uppercase", marginBottom: 16, display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ width: 3, height: 14, background: C.red, borderRadius: 2, display: "inline-block" }} />
        ...
        <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 8, padding: 20 }}>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              ...
              <Tooltip formatter={(v) => fmtBRL(Number(v))} contentStyle={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 6, fontSize: 12 }} />
        ...
        <ResponsiveContainer width="100%" height={160}>
          <BarChart data={chartData}>
            ...
            <Bar dataKey="EBITDA" radius={[4, 4, 0, 0]}>
              {chartData.map((d, i) => (
                <Cell key={i} fill={d.EBITDA < 0 ? C.red : C.blue} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
  ...
  return (
    <div style={{ background: C.gray50, minHeight: "100vh", fontFamily: "'Barlow',sans-serif" }}>
      ...
      <div style={{ background: C.white, borderBottom: `2px solid ${C.red}`, padding: "0 28px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", height: 56 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            >
              {["Consolidado", "Fibra", "Químicos"].map((f) => (
      ...
      <div style={{ background: C.dark, padding: "6px 28px", display: "flex", alignItems: "center", gap: 6 }}>
        ...
        <span style={{ marginLeft: "auto", color: C.gray600, fontSize: 10, fontFamily: "'JetBrains Mono',monospace" }}>🔴 DEMO — substituir por fetch ao Web App</span>
      </div>
      ...
      <div style={{ padding: "24px 28px", maxWidth: 1280, margin: "0 auto" }}>
        ...
        {tab === "dre" && (
          <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 8, overflow: "hidden" }}>
            <TabelaFinanceira rows={DRE_ROWS} mesSel={mesSel} titulo="DRE — 2025" mostrarAno={modoAnual} />
          </div>
        )}

        {tab === "dfc" && (
          <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 8, overflow: "hidden" }}>
            <TabelaFinanceira rows={DFC_ROWS} mesSel={mesSel} titulo="DFC — 2025" mostrarAno={modoAnual} />
          </div>
        )}
      </div>
